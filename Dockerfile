...
RUN mv stanford-corenlp-full-2015-12-09 renci_nlp_server/stanford-corenlp; cp renci_nlp_server/stanford-corenlp/stanford-corenlp-full-2015-12-09/*.jar renci_nlp_server/stanford-corenlp; mv stanford_corenlp_pywrapper-master renci_nlp_server/stanford_corenlp_pywrapper; cd renci_nlp_server; source ./bin/activate; cd stanford_corenlp_pywrapper; pip install .; cd ..; pip install -r requirements.txt; pip install requests; python -m nltk.downloader -d /usr/local/share/nltk_data wordnet


ENTRYPOINT ["/bin/bash", "renci_nlp_server/start.sh"]
#CMD ["/bin/bash"]`
